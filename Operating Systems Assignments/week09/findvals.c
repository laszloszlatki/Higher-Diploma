...
#include <omp.h>                                         // include to use openMP's #pragma omp
...
bool verbose = {false};                                   // create boolean value for verbose
 ...
 /* creating time for # StartTime and # EndTime */
  ...
  /* moved to after creating array and copiing numbers
  struct timeval tim;
  ...
  */
  ...
  int count = 0;
  #pragma omp parallel                                         // create team of parallel threads
  {
  #pragma omp for private (r,c) reduction(+:count)             // splits the team to handle the loop; r,c must be private in C; reduction     
    /*reduction (+:count) -> A private copy for each list variable is created and initialized for each thread.
      At the end of the reduction, the reduction variable is applied to all private copies of the shared variable,
      and the final result is written to the global shared variable. (https://computing.llnl.gov/tutorials/openMP/#REDUCTION)  */
    for (r = 0; r < rct; r++)
    {
    for (c = 0; c < cct; c++)
      {
  ...
  */
  ...
  printf("Search time: %.6lf(s)\n", t2-t1);                   // change "Elapsed time" to "Search time"
  exit(0);
